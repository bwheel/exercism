...
[<Task(1)>]
let ``Error message`` () =
    message "[ERROR]: Stack overflow" |> should equal "Stack overflow"

[<Fact>]
[<Task(1)>]
let ``Warning message`` () =
    message "[WARNING]: Disk almost full" |> should equal "Disk almost full"

[<Fact>]
[<Task(1)>]
let ``Info message`` () =
    message "[INFO]: File moved" |> should equal "File moved"
...
[<Task(2)>]
let ``Error log level`` () =
    logLevel "[ERROR]: Disk full" |> should equal "error"

[<Fact>]
[<Task(2)>]
let ``Warning log level`` () =
    logLevel "[WARNING]: Unsafe password" |> should equal "warning"

[<Fact>]
[<Task(2)>]
let ``Info log level`` () =
    logLevel "[INFO]: Timezone changed" |> should equal "info"
...
[<Task(3)>]
let ``Info reformat`` () =
    reformat "[INFO]: Disk defragmented" |> should equal "Disk defragmented (info)"
